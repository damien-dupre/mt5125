...
> `r faa("exclamation-triangle", animate="flash", speed="slow", color="red")` Categorical variables can be converted to Continuous and Continuous variables can be converted to Categorical.
...
Now, it is time to have a closer look at the method section ...
...
# Model Representation in the Method Section of Academic Reseach Papers

---

# Model Representation

.center[### A model is an overview of the predicted relationship between variables stated in the hypotheses.]

To draw a model, the following rules have to be applied:
...
.center[`r faa("exclamation-triangle", animate="flash", speed="slow", color="red")` By default, an interaction effect also involves the test of the main effect hypotheses of all Predictors included]
]
...
- **squares** are actual **measures**
- **circles/ovals** are **latent variables** calculated from a survey with multiple questions
...
Now, we can distinguish 2 types of relationship in a model, one for each type of hypothesis:
...
`r faa("exclamation-triangle", animate="flash", speed="slow", color="red")` Testing interaction effect hypotheses require a significantly higher sample size than testing main effect hypotheses.
...
A Mediation model is a complex path analysis between 3 variables, where one of them explains the relationship between the other two. It is usually used to identify cognitive processes in psychology.
...
```{r}
#| fig.cap: "Technology Acceptance Model (Davis, 1982)"
DiagrammeR::grViz("
digraph rmarkdown {
  graph [rankdir = LR]
...
# Understanding the Equation used to Test Hypotheses
...
Relationship between a $Predictor$ and an $Outcome$ variable (stated in a main effect hypothesis or in an interaction effect hypothesis) is called the **Estimate** $b$ and is analysed in terms of:
...
$$Outcome = b_0 + b_n \,Predictors + e$$

where the $Predictors$ includes all the $n$ variables used as predictor in formulated hypotheses using this specific $Outcome$ variable and being each associated to a specific $b$ estimate.
...
To evaluate if the strength of the relationship $b_n$ between a Predictor $n$ and an Outcome variable is significant, the full equation is statistically tested and $b_n$ estimated with a value.

The value of predictor's effect estimate $b_n$ **is the value of the slope** of the regression line between the predictor and the outcome.
...
The intercept coefficient $b_{0}$ is always included in an equation but its result is useless for hypothesis testing.
...
#### 2. Hypotheses
> Formulate your alternative hypotheses by using the proposed templates. Any other formulation, even if it make sense, is not good practice.

> An interaction hypothesis requires the formulation of the main effect hypothesis of each predictor involved.

#### 3. Model
> A model should represent all your hypotheses and only your hypotheses but draw only one model per Outcome variable (except is doing SEM analyses).
...
Now, it's time has come to test these hypotheses by using our equation(s)! 

```{r out.width="50%"}
knitr::include_graphics("img/tweet_linear.png")
